		...
		<div class="w-1/2">
			<div class="flex justify-center mb-4">
				<h1 class="mb-4 whitespace-nowrap text-center text-xl text-white md:text-2xl">
					<span class="inline-block rounded-2xl bg-black/5 px-4 py-4 font-semibold text-white ring-1 ring-yellow-400"><span class="text-white font-bold">Máximo <span class="text-yellow-400">(2-3 años)</span></span><br /> <br />Redistribución grasa corporal  <br /> Disminución pelo corporal  <br /> Disminución volumen testicular (50%)  <br /> Disminución producción de espermas  <br /> Tetas </span>
				</h1>
			</div>
			<div class="relative h-4 w-full overflow-hidden rounded-full bg-white">
				<div class="animate-stripes absolute left-0 top-0 h-full rounded-lg bg-yellow-400" :style="{ width: progressWidth }"></div>
			</div>
			<p class="mt-2 text-center text-2xl font-semibold text-white">
				<span class="font-bold text-yellow-400">{{ progressWithDecimals.slice(0, -9) }}</span
			...
			<p v-if="iniciado && !finalizado" class="mt-2 text-center text-2xl font-semibold text-white">
				<span  class="font-bold text-yellow-400">{{ daysRemaining }} días</span> restantes
			</p>
			<p v-else-if="!iniciado && !finalizado" class="mt-2 text-center text-2xl font-semibold text-white">
				<span  class="font-bold text-yellow-400">{{ startDaysRemaining }} días</span> para empezar
			</p>
		</div>
	</div>
</template>
